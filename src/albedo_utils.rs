...
pub fn average(images: &[DynamicImage]) -> Option<DynamicImage> {
    ...
    let images_average: Vec<u8> = images_sum
        .iter()
...
pub fn brightness_tilt(
    ...
    for y in 0..result.height() {
        for x in 0..result.width() {
            ...
            let relative_intensity = (upper_left * (1. - f_x) + upper_right * f_x) * (1. - f_y)
            ...
            for i in 0..pixel_data.len() - 1 {
                pixel_data[i] = (pixel_data[i] as f32 / relative_intensity).min(255.0) as u8;
            }
...
pub fn corner_flatten(image_data: &DynamicImage) -> DynamicImage {
    ...
    let bottom = radiance.rows_range(radiance.nrows() - size[0]..radiance.nrows());
    ...
    let average_intensity = (upper_left + upper_right + lower_left + lower_right) / 4.;
        ...
        .map(|i| {
            ...
            for x in 0..sub_image.width() {
                for y in 0..sub_image.height() {
                    ...
                    weight += (dx + dy) as f32 * sub_image.get_pixel(x, y).0[0] as f32;
                }
            }
            weight
        })
